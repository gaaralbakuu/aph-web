...
// Queries
const { data: menuData, refetch: refetchMenuList } = useQuery({
  queryKey: ['menuList', menuObj.query],
  queryFn: () => $request(api.videoServer + '/Video/VideoMenu/getlist', menuObj.query),
  enabled: false
})

watch(() => menuData.value, (newVal) => {
  if (newVal) {
    menuObj.list = newVal.data.list
  }
})

// Mutations
const addOrModifyMenuMutation = useMutation({
  mutationFn: (data) => $request(api.videoServer + '/Video/VideoMenu/addOrModifyMenu', data, 'post'),
  onSuccess: (r) => {
    if (r.status) {
...
const modifyMenuSeqMutation = useMutation({
  mutationFn: (data) => $request(api.videoServer + '/Video/VideoMenu/ModifyMenuSeq', data, 'post'),
  onSuccess: () => {
...
const deleteMenu = (data) => {
  ...
  $confirm(l.confirmDeleteMenu.replace('{name}', data.name_zh), l.deleteMenuTitle, {
...
const saveIndex = () => {
  ...
  modifyMenuSeqMutation.mutate(data)
}
