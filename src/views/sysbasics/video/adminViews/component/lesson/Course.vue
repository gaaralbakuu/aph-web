...
export default {
  ...
  methods: {
    ...
    getVideoList() {
      this.$request(this.$api.videoServer + '/Video/VideoManage/getVideoList', this.videoListObj.query).then((r) => {
        if (r.httpCode == 200) {
    ...
    getTagList() {
      this.$request(this.$api.videoServer + '/Video/VideoTag/getList', this.tagObj.query).then((r) => {
        if (r.httpCode == 200) {
    ...
    uploadCover() {
      ...
      this.$request(this.$api.videoServer + '/Video/VideoManage/uploadPicture', formData, 'post').then((r) => {
    ...
    modifyCourseStatus(i) {
      if (this.rightCheck(i, true)) {
        ...
        this.showObj.confirmData = {
          ...
          callback: () => {
            this.$request(
              this.$api.videoServer + '/Video/VideoCourseCatalog/modifyCourseStatus',
              {
                key: i.id,
                value: value,
              },
    ...
    getCourseList() {
      this.$request(this.$api.videoServer + '/Video/VideoCourseCatalog/getCourseList', this.courseObj.query, 'post').then((r) => {
    ...
    getAllCollegeList() {
      this.$request(this.$api.videoServer + '/Video/VideoCollege/getCollegeList', { page: 1, pageSize: 9999 }).then((r) => {
        this.publicCodeObj.allCollegeList = r.data.list
      })
    },

    getCollegeList() {
      this.$request(this.$api.videoServer + '/Video/VideoMenu/getCollegeRoleByPath', { resource_path: this.$route.path }).then((r) => {
        this.publicCodeObj.collegeList = [{ id: '', name_label: this.l.allChannels || 'All Colleges' }, ...r.data]
    ...
    getLanguage_type() {
      this.$request(this.$api.publiccode + 'getListdetailed', { queryString: {}, ruleno: '121', pageSize: 9999 }).then((r) => {
        let list = []
    ...
    getCourseCatalog() {
      this.$request(this.$api.publiccode + 'getListdetailed', { queryString: {}, ruleno: '122', pageSize: 9999 }).then((r) => {
        let list = []
        r.data.list.forEach((j) => {
          list.push({ label: j.name_label, value: j.code_no })
        })
