        ...
        <thead>
          <tr>
            <th v-for="(col, colIdx) in columns" :key="col.id" :style="getStickyStyle(col, colIdx, true)" :class="[col.className, ' px-2 py-3 font-medium text-sm text-black text-left whitespace-nowrap sticky top-0 z-10 transition-colors bg-white dark:bg-gray-900 dark:border-gray-700 dark:text-gray-300 border-b border-solid border-gray-200 tracking-wide', col.freeze ? 'sticky-' + col.freeze : '']">
              <div class="block max-w-full overflow-hidden overflow-ellipsis leading-5" :title="col.title === '#' ? '#' : l[col.title] || col.title">
...
function handleViewAttachments(row) {
  emit('view-attachments', row)
}

function getAuditResultType(result) {
  const typeMap = {
    pass: 'success',
    fail: 'danger',
    pending: 'warning',
...
function getRecStatusType(status) {
  const typeMap = {
    '-1': 'info',
    '1': 'warning',
    '7': 'success',
...
function getRecStatusText(status) {
  // Use loose equality as status might be number or string
...
function formatSurveyYear(value) {
  if (!value) return ''
...
function isInitialAssessment(value) {
