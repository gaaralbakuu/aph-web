      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500">{{ l.manageDocumentGuides }}</p>
          </div>
        </div>
        ...
        <el-table :data="data" v-loading="isLoading" class="w-full modern-table" highlight-current-row @row-click="handleRowClick" :row-style="{ cursor: 'pointer' }">
          ...
          <el-table-column prop="folder_name" :label="'Folder name'" width="140" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              <span v-else class="text-gray-400 text-sm">{{ l.noFolder }}</span>
            </template>
          </el-table-column>
...
const { l, c } = useLocalI18n('ShareHelpManualTable')
...
const emit = defineEmits(['action', 'row-click', 'add-click', 'file-preview', 'page-change'])
