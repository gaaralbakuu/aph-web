    ...
    <div class="flex-1 overflow-hidden">
      <div class="overflow-y-auto" style="max-height: 500px;">
        ...
        >
          ...
          <el-table-column 
            :label="$t('ShareHelpManualTable.serialNumber')" 
            width="80" 
            fixed="left"
          >
          ...
          <el-table-column 
            prop="file_name" 
            :label="$t('ShareHelpManualTable.fileName')" 
            min-width="200"
          ...
          >
            <template slot-scope="scope">
              <div class="flex items-center gap-2">
                ...
                <span 
                  class="text-blue-600 hover:text-blue-800 cursor-pointer font-medium hover:underline"
                  @click.stop="handleFilePreview(scope.row)"
                >
                  {{ scope.row.file_name }}
                </span>
              </div>
          ...
          <el-table-column 
            prop="file_suffix" 
            :label="$t('ShareHelpManualTable.fileType')" 
            width="100"
          >
            <template slot-scope="scope">
          ...
          <el-table-column 
            prop="create_time" 
            :label="$t('ShareHelpManualTable.uploadDate')" 
            width="120"
            show-overflow-tooltip
          >
            <template slot-scope="scope">
          ...
          <el-table-column 
            prop="description" 
            :label="$t('ShareHelpManualTable.description')" 
            min-width="150"
            show-overflow-tooltip
          >
            <template slot-scope="scope">
      ...
      <div v-if="!isLoading && (!data || data.length === 0)" class="flex flex-col items-center justify-center py-12">
        ...
        <button 
          v-show="showAuth.m_add" 
          @click="handleAddClick()" 
          class="mt-4 inline-flex items-center h-10 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-full px-4 shadow-sm transition duration-200 ease-in-out transform hover:scale-105 gap-2"
        >
          <i class="fa fa-plus text-sm"></i>
    ...
    <div v-if="data && data.length > 0" class="px-6 py-4 border-t border-gray-100 bg-gray-50">
      <div class="flex items-center justify-between">
        <div class="text-sm text-gray-500">
          {{ $t('ShareHelpManualTable.totalDocuments') }}: {{ total }}
        </div>
        <z-pagination 
