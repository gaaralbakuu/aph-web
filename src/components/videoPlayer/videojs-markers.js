...
function registerVideoJsMarkersPlugin(options) {
  ...
  var setting = jq.extend(true, {}, defaultSetting, options),
    markersMap = {},
    markersList = [],
    // list of markers sorted by time
    videoWrapper = jq(this.el()),
  ...
  function createMarkerDiv(marker) {
    var markerDiv = jq("<div class='vjs-marker'></div>");
    ...
    if (marker.color) {
      markerDiv.css('background-color', marker.color);
    }
    ...
    markerDiv.on('click', function(e) {
      ...
      if (!preventDefault) {
        var key = jq(this).data('marker-key');
  ...
  function registerMarkerTipHandler(markerDiv) {
    markerDiv.on('mouseover', function() {
      ...
      if (!!markerTip) {
        ...
        let markerLeft = getPosition(marker)
        let maxWidth = progressBarWidth * 0.5
        let width = textLength * 16 > maxWidth ? maxWidth : textLength * 16
        let left = markerLeft * progressBarWidth / 100
        let moveLeft = markerLeft * width / 100

        jq('.vjs-time-tooltip').first().css("visibility", "hidden")


        markerTip.find('.vjs-tip-arrow').css({
          "left":markerLeft+"%"
        })

        markerTip.css({
          "width": width + 20 + "px",
          "left": (left - moveLeft) + "px",
    ...
    markerDiv.on('mouseout', function() {
      ...
      jq('.vjs-time-tooltip').first().css("visibility", "visible")
    });
  }

  function initializeMarkerTip() {
    markerTip = jq(
      "<div class='vjs-tip'><div class='vjs-tip-arrow'></div><div class='vjs-tip-inner'><div class='vjs-tip-text'></div></div></div>"
      );
  ...
  function initializeOverlay() {
    breakOverlay = jq("<div class='vjs-break-overlay'><div class='vjs-break-overlay-text'></div></div>").css(setting
      .breakOverlay.style);
